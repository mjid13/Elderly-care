...
<h1 >Control Admin
<br>
<h4>people asking for Service</h4>
<br></h1>
...
<h3>services list</h3>
 ...
 include 'dbconnection.php';
			...
			$stmt = $conn->query("SELECT `serviceID`, `place`, `start_address`, `finish_distention_address`,`date`,`time`, `username`, `member_id`, `state`  FROM `service`  ");
			...
			if($n>0)  	
			{
				...
				echo "<tr>";
					...
					echo "<th>place</th>";
					echo "<th>start address</th>";
					echo "<th>finish distention address</th>";
					echo "<th>Date</th>";
					echo "<th>Time</th>";
					echo "<th>username</th>";
				...
				foreach($rows as $row)
				{
					echo "<tr>";
						...
						echo "<td>{$row['finish_distention_address']}</td>";
						echo "<td> {$row['date']} </td>";
						echo "<td> {$row['time']} </td>";
						echo "<td>{$row['username']}</td>";
						echo "<td>{$row['member_id']}</td>";
						echo "<td>{$row['state']}</td>";
						echo "<td><br> <form method='post'> <input name='helper_id' type='text' placeholder='Enter the member ID'/> <br> <input name='Accep' type='submit' value='Accept' /> <input name='rejec' type='submit' value='Reject' /></form> </td>" ;
						...
						{
							// UPDATE `service` SET `member_id`=33,`state`="good" WHERE `serviceID`=1
						  $sql="UPDATE `service` SET `member_id`=:memb where serviceID='{$row['serviceID']}'";
				          $stmt=$conn->prepare($sql);
				
				            $stmt->execute(array(
							':memb'=>$_POST['helper_id']
