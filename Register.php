...
<form method="post">
	...
	<tr>
		<td style="width: 110px">Civil_ID:</td>
		<td><input name="Civil" type="number" /> <?php if(isset($PHerror)){echo "*$CIerror";} ?></td>
	</tr>
		<tr>
		...
		<td>Male<input checked="checked" name="gender" type="radio" value="male" id="male" />&nbsp;&nbsp; 
		Female<input name="gender" type="radio" value="female" id="female" /></td>
		</tr>
		<tr>
		<td style="width: 130px">phone number:<?php if(isset($PHerror)){echo "$PHerror";} ?></td>
		...
		<td><input name="address" type="text" placeholder="example=Muscat" /></td>
		</tr>
</table>


	<input name="Sub1" type="submit" value="save" /><input name="Reset1" type="reset" value="reset" />
	...
	<?php
		...
		if(isset($_POST['save']))
				...
				
			{
				...
				$sql="INSERT INTO `users`(`id`, `username`, `password`,`User_control`,`Email`, `Gender`, `Phone_Number`, `address`) VALUES (:civ,:use,:pas,0,:ema,:gen,:pho,:add)";
				$stmt=$conn->prepare($sql);
				//or
				//$stmt=$conn->prepare("insert into student values(:sid, :sn, :saddr)");
				$stmt->execute(array(
				':civ'=>$_POST['Civil'],
				':pas'=>$enpassword,
				':use'=>$_POST['username'],
				...
				$verify = password_verify($_POST['password'], $enpassword);
			    if ($verify)
			    {
